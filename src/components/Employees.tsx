...
function getIcon(field: SortField, sortOptions: SortByFieldsStore): ReactNode {
  ...
  if (order != "no") {
    result =
      order == "asc" ? <FaSortUp> </FaSortUp> : <FaSortDown></FaSortDown>;
  }
  return (
    <IconButton
      size="xs"
          ...
          >
            <Table.Header>
              <Table.Row bg="bg.subtle">
                ...
                <Table.ColumnHeader data-sticky="end" minW="160px" left="16">
                  Full Name {getIcon("fullName", sortOptions)}
                </Table.ColumnHeader>
                <Table.ColumnHeader>
                  Department{getIcon("department", sortOptions)}
                </Table.ColumnHeader>
                <Table.ColumnHeader>
                  Salary{getIcon("salary", sortOptions)}
                </Table.ColumnHeader>
                <Table.ColumnHeader>
                  Birthdate{getIcon("birthdate", sortOptions)}
                </Table.ColumnHeader>
            ...
            <Table.Body>
              {sortedEmployees.map((empl) => (
                <Table.Row key={empl.id}>
                  ...
                  <Table.Cell data-sticky="end" left="16">
                    {empl.fullName}
                  </Table.Cell>
